...
class TestCensusAPI:
    ...
    def test_rate_limiting(self):
        ...
        with patch('pytidycensus.api.time.time') as mock_time, \
             patch('pytidycensus.api.time.sleep') as mock_sleep:
    ...
    def test_get_variables_success(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"variables": {"B01001_001E": {"label": "Total population"}}}
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
    ...
    def test_json_decode_error_handling_geography_codes(self, mock_get):
        ...
        with pytest.raises(ValueError, match="Census API returned invalid response for geography codes"):
            api.get_geography_codes(2022, "acs", "acs5")
