...
from pytidycensus.estimates import get_estimates, get_estimates_variables, _add_breakdown_labels
...
class TestGetEstimates:
    ...
    def test_get_estimates_validation_errors(self, mock_api_class):
        ...
        with patch('pytidycensus.estimates.process_census_data') as mock_process:
            ...
            with pytest.raises(ValueError):
                get_estimates(geography="state", variables="POP", year=1999, api_key="test")
    ...
    def test_get_estimates_different_years(self, mock_api_class):
        ...
        with patch('pytidycensus.estimates.process_census_data') as mock_process:
            ...
            for year in [2020, 2021, 2022]:
                get_estimates(geography="state", variables="POP", year=year, api_key="test")
    ...
    def test_get_estimates_api_error(self, mock_api_class):
        ...
        with pytest.raises(Exception, match="Failed to retrieve population estimates: API request failed"):
            get_estimates(geography="state", variables="POP", api_key="test")
    ...
    def test_get_estimates_different_outputs(self, mock_api_class):
        ...
        with patch('pytidycensus.estimates.process_census_data') as mock_process:
            ...
            # Test tidy output
            get_estimates(geography="state", variables="POP", output="tidy", api_key="test")
            call_args = mock_process.call_args[0]
            assert "tidy" in call_args
            
            # Test wide output
            get_estimates(geography="state", variables="POP", output="wide", api_key="test")
            call_args = mock_process.call_args[0]
    ...
    def test_get_estimates_breakdown_labels_processing(self, mock_api_class):
        ...
        with patch('pytidycensus.estimates.process_census_data') as mock_process, \
             patch('pytidycensus.estimates._add_breakdown_labels') as mock_add_labels:
            mock_process.return_value = mock_df
...
class TestAddBreakdownLabels:
    ...
    def test_add_breakdown_labels_agegroup(self):
        ...
        assert result["AGEGROUP_label"].tolist() == ["0-4 years", "5-9 years", "85+ years"]
    ...
    def test_add_breakdown_labels_race(self):
        ...
        expected = ["White alone", "Black or African American alone", "Two or More Races"]
    ...
    def test_add_breakdown_labels_hisp(self):
        ...
        assert result["HISP_label"].tolist() == ["Not Hispanic or Latino", "Hispanic or Latino"]
    ...
    def test_add_breakdown_labels_multiple(self):
        ...
        assert "RACE_label" in result.columns
        assert result["SEX_label"].tolist() == ["Male", "Female"]
        assert result["RACE_label"].tolist() == ["White alone", "Black or African American alone"]
