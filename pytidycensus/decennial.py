...
def get_decennial(
    ...
    **kwargs
    ...
    if shift_geo and not geometry:
        raise ValueError("`shift_geo` is only available when requesting feature geometry with `geometry = TRUE`")
    ...
    if sumfile == "sf3":
        ...
        if year > 2001:
            raise ValueError("Summary File 3 was not released in 2010. Use Summary File 1 or tables from the American Community Survey via get_acs() instead.")
    elif sumfile == "sf1":
        ...
        if not pop_group:
            raise ValueError("You must specify a population group to use the DDHC-A file. Specify `pop_group` parameter.")
    ...
    try:
        ...
        if variable_names and output == "tidy":
            ...
            rename_dict = {code: name for name, code in variable_names.items() if code in df.columns}
        ...
        if summary_var:
            if output == "tidy":
                ...
                summary_df = df[df['variable'] == summary_var][['GEOID', 'value']].copy()
        ...
        missing_values = [-111111111, -222222222, -333333333, -444444444, 
