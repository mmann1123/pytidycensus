    ...
    if cache and os.path.exists(cache_path):
        try:
            with open(cache_path, 'rb') as f:
                df = pickle.load(f)
            print(f"Loaded cached variables for {year} {dataset}" + (f" {survey}" if survey else ""))
            return df
...
def _parse_variables(variables_data: Dict[str, Any]) -> pd.DataFrame:
    ...
    if len(df) == 0:
        return pd.DataFrame(columns=['name', 'label', 'concept', 'predicateType', 'group', 'limit', 'table'])
...
def list_available_datasets(year: int) -> Dict[str, list]:
    ...
    datasets = {
        ...
        "dec": ["sf1", "sf2", "sf3", "sf4"] if year <= 2000 else ["sf1"] if year == 2010 else ["pl"] if year == 2020 else [],
