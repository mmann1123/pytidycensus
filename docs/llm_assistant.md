...
- **Wide Format Output**: Always returns clean DataFrames with simplified column names
...
After installation, run the CLI with your API keys:
...
### LLM Dependencies
To use the LLM assistant, you need either OpenAI or a local model like Ollama. In order to get an API key for OpenAI, [sign up here](https://platform.openai.com/)signup or [watch this video](https://youtu.be/dJ5aIRUyhNA?si=KXKoFYkQk4rpsUcx) 
...
### Census API Key
Get a free Census API key and set it an an environment variable:
```bash
...
- **Total Population by State (2010)**: Decennial census data
- **Wisconsin County Income**: ACS county-level analysis
- **Dane County Tract Income**: Neighborhood-level analysis
- **Georgia Counties with Custom Names**: Variable renaming
- **CBSA Population Analysis**: Metropolitan area data
- **DC Inequality Analysis**: Normalization variables demo
- **Spatial Wisconsin Income**: Geometry for mapping
- **Age/Sex Table Analysis**: Full table queries
...
- Use the CLI for interactive exploration: `python -m pytidycensus.llm_interface.cli`
- Check conversation state: `assistant.get_conversation_state()`
- Export conversations: `assistant.export_conversation()`
- Reset if stuck: `assistant.reset_conversation()`
 
## Performance Notes

- OpenAI calls: ~$0.01-0.02 per conversation
- Local Ollama: Free but requires 4GB+ RAM
- First variable search may be slower (builds cache)
- Wide format + cleaning adds minimal overhead
